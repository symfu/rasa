...
def _fingerprint(
    ...
    return {
        ...
        FINGERPRINT_CONFIG_CORE_KEY: config_core if config_core is not None else ["test"],
        FINGERPRINT_CONFIG_NLU_KEY: config_nlu if config_nlu is not None else ["test"],
        FINGERPRINT_DOMAIN_WITHOUT_TEMPLATES_KEY: domain if domain is not None else ["test"],
...
def test_core_fingerprint_changed(fingerprint2, changed):
    ...
    assert section_fingerprint_changed(fingerprint1, fingerprint2, SECTION_CORE) is changed
...
def test_nlu_fingerprint_changed(fingerprint2, changed):
    fingerprint1 = _fingerprint()
    assert section_fingerprint_changed(fingerprint1, fingerprint2, SECTION_NLU) is changed
